...
{	
	LOG_WRITE_INFO("DownDataRestorer constructed ok");
}

DownDataRestorer::~DownDataRestorer()
{
	LOG_WRITE_INFO("DownDataRestorer descontruction");
...
int DownDataRestorer::Init(const string& redis_server_ip, uint16_t redis_server_port, int worker_thread_num, uint32_t connect_timeout_ms, uint32_t read_timeout_ms, const string& passwd)
{
	...
	std::stringstream log_msg;
	log_msg<<"DownDataRestorer init now, redis ip "<<redis_server_ip<<", redis port "<<redis_server_port<<", worker_thread num "<<worker_thread_num;
...
int DownDataRestorer::Init(RedisMode redis_mode, const REDIS_SERVER_LIST& redis_server_list, const string& master_name, int worker_thread_num, uint32_t connect_timeout_ms, uint32_t read_timeout_ms, const string& passwd)
{
	...
	if(redis_mode_==STAND_ALONE_OR_PROXY_MODE)
		LOG_WRITE_INFO("redis mode is STAND_ALONE_OR_PROXY_MODE");
	else if(redis_mode_==CLUSTER_MODE)
		LOG_WRITE_INFO("redis mode is CLUSTER_MODE");
	else if(redis_mode_==SENTINEL_MODE)
		LOG_WRITE_INFO("redis mode is SENTINEL_MODE");
	...
	log_msg<<"worker_thead num is "<<worker_thread_num_<<", ";
...
int DownDataRestorer::Uninit()
{
	...
	LOG_WRITE_INFO("DownDataRestorer Uninit now");
	inited_=false;
	return DDR_OK;
}
...
int DownDataRestorer::Start()
{
	if(!inited_)
	{
		LOG_WRITE_WARNING("please init DownDataRestorer first");
		return DDR_FAIL;
	}
	...
	LOG_WRITE_INFO("DownDataRestorer Start now");
...
int DownDataRestorer::Stop()
{
	if(!started_)
	{
		LOG_WRITE_WARNING("DownDataRestorer not started");
		return DDR_FAIL;
	}
	LOG_WRITE_INFO("DownDataRestorer stop now");
...
{	
	...
	LOG_WRITE_INFO("DownDataRestorer stat: ");

	std::stringstream log_msg2;
	log_msg2<<"device count: "<<GetDeviceCount();
	LOG_WRITE_INFO(log_msg2.str());

	std::stringstream log_msg3;
	log_msg3<<"channel count: "<<GetChannelCount();
	LOG_WRITE_INFO(log_msg3.str());
	
	for(int i=0; i<worker_thread_num_; i++)
	{
		std::stringstream log_msg;
		log_msg<<"ExecutingInviteCmdList["<<i<<"] count: "<<GetExecutingInviteCmdCount(i);
		LOG_WRITE_INFO(log_msg.str());
	}
	return DDR_OK;
}
...
//	CHECK((int)executing_invite_cmd_lists.size()==GetWorkerThreadNum())<<"ExecutingInviteCmdList size must equal to worker_thread_num "<<worker_thread_num_;
	if(static_cast<int>(executing_invite_cmd_lists.size())!=GetWorkerThreadNum())
	{
		std::stringstream log_msg;
		log_msg << "ExecutingInviteCmdList size must equal to worker_thread_num "<<worker_thread_num_;
		LOG_WRITE_ERROR(log_msg.str());
		return DDR_FAIL;
	}
