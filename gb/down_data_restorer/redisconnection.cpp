...
RedisConnection::RedisConnection(const string serverIp,uint32_t serverPort, uint32_t connectTimeout, uint32_t readTimeout)
{
	m_serverIp = serverIp;
...
bool RedisConnection::connect()
{
	if (!m_socket.connect(m_serverIp, m_serverPort, m_connectTimeout))
	{
		...
		log_msg<<"connection:["<<this<<"] connect to server:["<<m_serverIp<<":"<<m_serverPort<<"] failed.";
...
{
	std::stringstream log_msg;
	log_msg<<"connection:["<<this<<"] start to parse redis response:["<<parseBuf<<"], parseLen: "<<parseLen;
	LOG_WRITE_INFO(log_msg.str());
