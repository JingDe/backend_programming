...
public:
	Glogger()
	{
		...
		FLAGS_log_dir = "/home/wenrong/wangjing_tmp/downdatarestorerlogdir";
...
void TestControlQueueSize()
{
	int operation_size = sizeof(DataRestorerOperation);
	std::stringstream log_msg;
	log_msg << "DataRestorerOperation size is " << operation_size;
	LOG_WRITE_INFO(log_msg.str());

	// 30MB
	int max_bytes = 30 * 1024 * 1024;
	int max_count = max_bytes / operation_size;
	log_msg.str("");
	log_msg << "max count is " << max_count;
	LOG_WRITE_INFO(log_msg.str());

	int channel_size = sizeof(Channel);
	log_msg.str("");
	log_msg << "channel size is " << channel_size;
	LOG_WRITE_INFO(log_msg.str());

	int device_size = sizeof(Device);
	log_msg.str("");
	log_msg << "Device with nullptr size is " << device_size;
	LOG_WRITE_INFO(log_msg.str());

	int invite_size = sizeof(ExecutingInviteCmd);
	log_msg.str("");
	log_msg << "invite with nullptr size is " << invite_size;
	LOG_WRITE_INFO(log_msg.str());
}
