...
// only copy deviceId and channelDeviceId of device.channelList
static void CopyChannelIdList(Device* copy, Device* device)
{
	copy->channelList = std::make_unique<CChannelList>();
	if(copy->channelList == nullptr  ||  device->channelList == nullptr)
	{
		return;
	}

	std::list<ChannelPtr>& channel_ptr_list = device->channelList->GetChannelList();
	for (std::list<ChannelPtr>::iterator it = channel_ptr_list.begin(); it != channel_ptr_list.end(); ++it)
	{
		ChannelPtr channel_ptr(std::make_unique<Channel>());
		if (channel_ptr == nullptr)
			continue;
		channel_ptr->deviceId = (*it)->deviceId;
		channel_ptr->channelDeviceId = (*it)->channelDeviceId;
		copy->channelList->Add(std::move(channel_ptr));
	}
}
...
Device* CopyDevice(Device* d)
{
	...
	device->viaIp = d->viaIp;
	device->viaPort = d->viaPort;
	device->domain = d->domain;
