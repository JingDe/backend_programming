...
public:
	...
	RedisConnection(const string serverIp, uint32_t serverPort, uint32_t connectTimeout, uint32_t readTimeout);
	~RedisConnection();
	bool connect();
...
private:
	...
	bool   	m_valid;
	bool 	m_canRelease;
	int m_connectTimeout; // milliSeconds, default 900ms
	int m_readTimeout; // milliseconds, default 3000ms
};
