...
bool RedisClient::initSentinels()
{
	...
	for(REDIS_SERVER_LIST::const_iterator it=m_sentinelList.begin(); it!=m_sentinelList.end(); ++it)
	{
		...
		else
		{
			...
			{
				log_msg.str("");
				log_msg<<"init sentinel:["<<sentinalHandler.proxyId<<"] connect pool failed.";
				LOG_WRITE_ERROR(log_msg.str());
			}
			else
			{
		...
		{
			log_msg.str("");
			log_msg<<"sentinel "<<sentinalHandler.proxyId<<" subscribe +switch-master failed";
			LOG_WRITE_ERROR(log_msg.str());
		}
		else
		{
...
//			sentinalHandler.subscribed=true;
	...
	log_msg.str("");
	log_msg<<"config sentinel size is "<<m_sentinelList.size()<<", connected sentinel is "<<aliveSentinel;
	LOG_WRITE_INFO(log_msg.str());
...
//		client->CheckSentinelGetMasterAddrByName();
	...
	LOG_WRITE_INFO("sentinel health check task finish, exit");
...
bool RedisClient::DoSwitchMaster(const RedisServerInfo& masterAddr)
{
	...
	{
		std::stringstream log_msg;
		log_msg<<"master "<<m_masterClusterId<<" has connected";
		LOG_WRITE_INFO(log_msg.str());
		return true;
	}
...
//	WriteGuard guard(m_rwProxyMutex);
	if (m_redisProxy.clusterHandler != NULL)
	{
		LOG_WRITE_INFO("freeRedisProxy call freeConnectionPool");
...
bool RedisClient::ParseSentinelSlavesReply(const CommonReplyInfo& replyInfo, vector<RedisServerInfo>& slavesInfo)
{
	...
	{
		std::stringstream log_msg;
		log_msg<<"get the "<<i<<" Array, size is "<<replyInfo.arrays[i].size();
		LOG_WRITE_INFO(log_msg.str());
...
//	const string& replicationMsg=replyInfo.resultString;
	...
	log_msg.str("");
	log_msg<<"info replication reply msg: ["<<replicationMsg<<"]";
	LOG_WRITE_INFO(log_msg.str());
...
bool RedisClient::getClusterIdBySlot(uint16_t slotNum,string & clusterId)
{
	...
	if (m_slotMap.find(slotNum) != m_slotMap.end())
	{
		...
		std::stringstream log_msg;
		log_msg<<"slot:"<<slotNum<<" hit clusterId: "<<clusterId;
		LOG_WRITE_INFO(log_msg.str());
		return true;
	}
...
{
	LOG_WRITE_INFO("doTransactionCommandInConnection");
	RedisReplyInfo replyInfo;
	bool needRedirect;
	string redirectInfo;
    if(m_redisProxy.clusterHandler==NULL)
