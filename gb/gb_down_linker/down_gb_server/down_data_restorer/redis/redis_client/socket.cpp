...
bool Socket::__connectNonBockv4(InetAddr address, int port, int milliseconds){
	...
	if( ( n = ::connect(fd,(struct sockaddr *)&client_addr, client_len ))<0){
		if( errno != EINPROGRESS ){
			std::stringstream log_msg;
			log_msg<<"connect to "<<address.getHostAddress()<<":"<<port<<" faile: "<<errno<<", "<<strerror(errno);
			LOG_WRITE_ERROR(log_msg.str());
			return false;
		}
	}
...
int Socket::read2(void * buf, size_t maxlen, int secTimeout)
{
	...
	if (secTimeout > 0) {
		...
		if (ret == -1) {
			...
			log_msg2<<"fd="<<fd<<", tv_sec="<<tv.tv_sec<<", tv_usec=%d"<<tv.tv_usec;
			LOG_WRITE_ERROR(log_msg2.str());
...
bool Socket::WatchReadEvent(int& epollfd)
{
	...
	{
		std::stringstream log_msg;
		log_msg<<"epoll_create failed: "<<errno;
		LOG_WRITE_ERROR(log_msg.str());
		return false;
	}
	...
	{
		std::stringstream log_msg;
		log_msg<<"epoll_ctl ADD failed, fd "<<fd<<", "<<errno;
		LOG_WRITE_ERROR(log_msg.str());
		::close(epfd);
		return false;
	}
...
bool Socket::WaitReadEvent(int epollfd)
{
	...
	if(nEvents!=1)
	{
		std::stringstream log_msg;		
		if(errno==EINTR)
			log_msg<<"epoll_wait interrupted";
		else
