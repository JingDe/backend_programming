				...
				DBSerialize* serial, int* count)
{
	switch (commandType)
	{
		...
		case RedisCommandType::REDIS_COMMAND_EXISTS:
			...
			{
				std::stringstream log_msg;
				log_msg<<"find key:"<<key<<" in redis db";
				LOG_WRITE_ERROR(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
		...
		case RedisCommandType::REDIS_COMMAND_DEL:
			...
			{
				std::stringstream log_msg;
				log_msg<<"del key:"<<key<<" from redis db success.";
		...
		case RedisCommandType::REDIS_COMMAND_EXPIRE:
			...
			{
				std::stringstream log_msg;
				log_msg<<"set key:"<<key<<" expire success.";
				LOG_WRITE_INFO(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
		...
		case RedisCommandType::REDIS_COMMAND_ZADD:
			...
			{
				std::stringstream log_msg;
				log_msg<<"zset key:"<<key<<" add success.";
				LOG_WRITE_INFO(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
		...
		case RedisCommandType::REDIS_COMMAND_ZREM:
			...
			{
				std::stringstream log_msg;
				log_msg<<"set key:%s rem success."<<key;
				LOG_WRITE_INFO(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
			...
			{
				std::stringstream log_msg;
				log_msg<<"set key:"<<key<<" rem failed.";
				LOG_WRITE_ERROR(log_msg.str());
				freeReplyInfo(replyInfo);
				return false;
			}
		...
		case RedisCommandType::REDIS_COMMAND_ZINCRBY:
			...
			{
				std::stringstream log_msg;
				log_msg<<"set key:"<<key<<" zincrby success.";
				LOG_WRITE_ERROR(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
		...
		case RedisCommandType::REDIS_COMMAND_ZREMRANGEBYSCORE:
			...
			{
				std::stringstream log_msg;
				log_msg<<"set key:"<<key<<" zremrangebyscore success.";
				LOG_WRITE_INFO(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
		...
		case RedisCommandType::REDIS_COMMAND_SISMEMBER:
			...
			if(replyInfo.replyType == RedisReplyType::REDIS_REPLY_INTEGER)
			{
				...
				std::stringstream log_msg;
				log_msg<<"key "<<key<<" commandType "<<commandType<<" success, return integer "<<replyInfo.intValue;
				LOG_WRITE_INFO(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
...
//				CreateConnectionPool(it);
			...
			{
				std::stringstream log_msg;
				log_msg<<"succeed to send to slave "<<it->second.proxyId;
				LOG_WRITE_INFO(log_msg.str());
				success=true;
				break;
			}
...
REDIS_COMMAND:
	...
	log_msg.str("");
	log_msg<<"start to parse command type:"<<commandType<<" redis reply.";
	LOG_WRITE_INFO(log_msg.str());
	...
	switch (commandType)
	{
		...
		case RedisCommandType::REDIS_COMMAND_ZREM:
			...
			{
				log_msg.str("");
				log_msg<<"set key:"<<key<<" rem success.";
				LOG_WRITE_INFO(log_msg.str());
				freeReplyInfo(replyInfo);
				return true;
			}
		...
		case RedisCommandType::REDIS_COMMAND_ZSCORE:
			...
			{	
				...
				if(count)
					*count = atoi(score_c);
				log_msg.str("");
				log_msg<<"key:"<<key<<", score:"<<atoi(score_c);
				LOG_WRITE_INFO(log_msg.str());
				return true;	
			}
			break;
...
bool RedisClient::Set(RedisConnection* con, const string & key, const DBSerialize& serial)
{
	...
	fillCommandPara(key.c_str(), key.length(), paraList);
	paraLen += key.length() + 20;	
	LOG_WRITE_INFO("Set DBSerialize");
	DBOutStream out; 
	//serial.save(out);
	save(serial, out);
	std::stringstream os;
	os<<out.getSize();
	LOG_WRITE_INFO(os.str());
	fillCommandPara(out.getData(), out.getSize(), paraList);
	paraLen += out.getSize() + 20;
